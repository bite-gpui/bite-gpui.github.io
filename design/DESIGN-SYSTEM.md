...
| Nav pipes               | —                                                                  | 1px × 16px `--border-2`                                       | —                                                                                                                                        | The bar reads as one sentence in pipes: the mark · the way back · the section you are in · everything else · the install line. On a sub-page the first three are the whole of it — a reader inside one section is not helped by the guides that stand beside it, which are rows in the sheet and in the footer on every page — and the crumb between them carries the trail: the step up, then the section's name with its brand dot. The rail beside it is the section's pages, the hub first under the name a reader gets there by (`.nav-back-group::before`/`::after` for the pipes; `.nav-divider` splits the frontdoor's chapters from its guides). Pseudo-elements rather than flex items — as items each would cost one of the bar's own 32px gaps                                                                                                                                      |
| Current page marker     | solid `--text`                                                     | —                                                             | `--bg`                                                                                                                                   | Fill, not hue, the way the dock switches carry state: the marker never spends the `--brand` that the install CTA and every affordance carries, so it is a light pill with dark ink here and a dark one with light ink on the sage paper. Padded, with a matching negative margin so the padding never grows the box it marks: padding the marker and keeping it shoves the row along, and on the one-pager the mark moves on every scroll. (A spread `box-shadow` is paint-only too, but Chrome paints an inline box's own shadow over its text, so the label comes out blank.) `aria-current="page"` on a sub-page; `aria-current="location"` for the one-pager's scroll spy. The section's dot reads back out of the fill when the pill lands on the hub it marks. The sheet stacks its rows, so there the marker is the same fill squared off, where nothing sits beside the row to be moved |
...
| Width   | What changes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ≤1180px | The nav's rail and CTA move into a drop-down sheet, leaving the mark, the way back and the two controls in the bar. It has its own breakpoint, above the others: the frontdoor row carries seven links plus the CTA since the guide pages were added, which needs a wider bar than the four anchors did, so it no longer shares 820px with the hero. On a sub-page the rest of the site goes into the sheet, where the section's own pages are rows beside the guides; the bar keeps the mark, the way back and the section the reader is in. The sheet is a separate layer, so a page still in desktop layout underneath it is fine. |
...
| ≤900px  | A sub-page's bar gives up the version chip, then the wordmark at ≤560px (the mark and "← Frontdoor" are the two that never go), then at ≤440px the pipes around the crumb and the section's name with them; the frontdoor gives up its chip at ≤420px instead. Measured through a 320px frame: a section bar is 251px of the 274px it has.                                                                                                                                                                                                                                                                                            |
| ≤820px  | The scroll-linked hero is re-composed for portrait: the copy stacks above the stack, the burger is re-based to the foot of its frame and pinned to the foot of the viewport, and the pinned journey drops from 280vh to 200vh. The guide pages stack their two- and three-up card grids.                                                                                                                                                                                                                                                                                                                                              |
| ≤640px  | The stepper and the terminal head wrap, and the hero copy compacts and gives up its eyebrow pill. The guide-page hero tightens; their data tables keep a 620px floor and scroll inside `.table-scroll` rather than reflowing.                                                                                                                                                                                                                                                                                                                                                                                                         |
...
existed — one list, no duplicated markup to keep in sync. On a sub-page that list is the sheet's alone
(`.nav-links.is-sheet-only`), because the bar above already carries the section the
reader is in: one list, reachable at every width, and never rendered twice.
