...
| Nav pipes               | —                                                                  | 1px × 16px `--border-2`                                       | —                                                                                                                                        | The bar reads as one sentence in pipes: mark and version · where you are and what is under it · everything else · the install line. One pipe each side of the trail on a sub-page (`.nav-trail::before`/`::after`), one between the chapters and the guides on the frontdoor (`.nav-divider`). Pseudo-elements rather than flex items — as items each would cost one of the bar's own 32px gaps                                 |
| Current page marker     | `--surface`, ringed with `--border-2`                              | —                                                             | `--text`                                                                                                                                 | A pill painted as a spread `box-shadow`, never padding and a border: a padded pill grows the box it marks and shoves the row along, and on the one-pager the mark moves on every scroll. `aria-current="page"` on a sub-page; `aria-current="location"` for the one-pager's scroll spy. The sheet stacks its rows, so there the marker is a `--surface-2` band plus `600` weight, where nothing sits beside the row to be moved |
...
| ≤900px  | A sub-page's bar gives up the version chip, then the wordmark at ≤560px (the mark and "← Frontdoor" are the two that never go), and the frontdoor's chip at ≤420px. Measured through a 320px frame: a section bar is 245px of the 274px it has.                                                                                                                                                                                                                                                                                 |
