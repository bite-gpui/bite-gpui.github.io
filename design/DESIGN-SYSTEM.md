...
| Nav pipes               | —                                                                  | 1px × 16px `--border-2`                                       | —                                                                                                                                        | The bar reads as one sentence in pipes: the mark · the way back · the section you are in · everything else · the install line. A sub-page's way back is piped on both sides (`.nav-back-group::before`/`::after`) and its section closes with one only when it holds pages one level in (`.nav-trail.has-nested::after`); the frontdoor splits chapters from guides (`.nav-divider`). Pseudo-elements rather than flex items — as items each would cost one of the bar's own 32px gaps                                                                                                                                                                                                                                                                                                                                                                                                          |
...
existed — one list, no duplicated markup to keep in sync. The one entry the rail
drops on desktop (`li[data-in-trail]`, a page the trail above it already names) is
put back in the sheet, so a section's pages stay reachable at every width without
